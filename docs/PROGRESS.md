...
- H5 完成首页视觉重设和全站说明性文案瘦身：赛事入口页顶部改为独立 MRJZ 品牌 banner，背景使用两条对向滚动的全英雄头像轨道并保留快捷入口，真实比赛比分回到赛事入口卡片中展示；次级页面保留数据本身，移除面向实现说明的冗余文字。
...
4. 将 Web Admin 从原生 DOM 原型升级为正式组件结构；H5 继续补路由级加载、错误和空状态细节。
