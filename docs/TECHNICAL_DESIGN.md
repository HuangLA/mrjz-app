...
- 普通数据称号按场均值排序；购买次数、暴走次数、指定英雄选择次数和击杀痛苦魔方等触发类称号按本届总次数排序；`PlayBoy` 按本届不同英雄数排序；候选人必须在该届至少参赛 5 场。
- 返回每项称号获得者和候选前五，同时返回场均值或聚合值、总计和参赛场数。
- H5 和小程序不得自行重算英雄榜，只消费 `/api/tournaments/:id/hero-leaderboards`；特殊触发称号说明由后端返回“触发条件隐藏”。
...
- 标签长度限制为 2 到 24 个可见字符，允许中文、英文、数字、常见标点、符号和 emoji；控制字符等不可见输入不可提交。
...
WECHAT_LOGIN_RATE_LIMIT_MAX=
WECHAT_LOGIN_RATE_LIMIT_WINDOW_MS=
MRJZ_RATE_LIMIT_TRUST_PROXY=
...
- `NODE_ENV=production` 时禁止启用 `MRJZ_ALLOW_DEV_WECHAT_LOGIN`。
- `MRJZ_ALLOWED_ORIGINS` 生产环境必须配置具体 H5 / Admin 域名，不能使用 `*`。
- 初次启动生产数据库前必须显式配置强 `ADMIN_INITIAL_PASSWORD`，不能使用 `change-me`。
- 若 API 位于 Nginx 之后，仅当代理层会覆盖 `X-Forwarded-For` 时才设置 `MRJZ_RATE_LIMIT_TRUST_PROXY=1`。
