...
英雄榜和单场趣味称号使用相同 OpenDota 字段，但统计口径不同：英雄榜按单届赛事聚合，普通数据称号按场均值排名，购买次数、暴走次数、指定英雄选择次数和击杀痛苦魔方等触发类称号按本届总次数排名，`PlayBoy` 按本届不同英雄数排名，并且只统计该届参赛 5 场及以上的选手。展开态展示每项候选前五、场均值或聚合值、总计和参赛场数。
