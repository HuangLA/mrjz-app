...
英雄榜和单场趣味称号使用相同 OpenDota 字段，但统计口径不同：英雄榜按单届赛事聚合，普通数据称号按场均值排名，购买次数、暴走次数、指定英雄选择次数和击杀痛苦魔方等触发类称号按本届总次数排名，`PlayBoy` 按本届不同英雄数排名，并且只统计该届参赛 5 场及以上的选手。场均值与总计并列时，先达到该数值（最后一次产生贡献的比赛时间更早）的选手排在前面，其次按参赛场数决胜。展开态展示每项候选前五、场均值或聚合值、总计和参赛场数。
...
| 反侦察科科长 | `players[].observer_kills` | 场均排掉观察守卫最多，岗哨守卫不计入，数值单位显示为“个” |
| 动物园园长 | `players[].camps_stacked` | 场均屯野最多，数值单位显示为“波” |
| 老毒物 | `players[].hero_id in [40, 47, 79]` | 剧毒术士、冥界亚龙、暗影恶魔选择次数最多；H5 和小程序说明显示“触发条件隐藏” |
| 止痛药 | `objectives[].type = CHAT_MESSAGE_MINIBOSS_KILL` 按 `player_slot` 归因 | 本届击杀痛苦魔方最多；H5 和小程序说明显示“触发条件隐藏” |
| 化龙 | `players[].hero_id in [49, 112, 47, 64, 13]` | 龙骑士、寒冰飞龙、冥界亚龙、杰奇洛、帕克选择次数最多；H5 和小程序说明显示“触发条件隐藏” |
| 老阴B | `players[].hero_id in [32, 62, 88, 56, 16, 46, 74, 63, 83, 9, 145]` | 力丸、赏金猎人、司夜刺客、骷髅射手、沙王、圣堂刺客、祈求者、编织者、树精卫士、月之女祭司、凯选择次数最多；H5 和小程序说明显示“触发条件隐藏” |
| KS | `players[].hero_id in [61, 16, 88]` | 育母蜘蛛、沙王、司夜刺客选择次数最多；H5 和小程序说明显示“触发条件隐藏” |
